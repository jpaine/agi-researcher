...
Two passes by Jeffrey. This sheet is the only labeling view.
...
Planning estimate, not a timed study: about 2–4 minutes per row per pass to read the step against the ten cards. For 60 rows that is about 2–4 hours per pass, plus the 72-hour gap before pass 2.
...
Rebuild the blank sheet from candidates with:

```bash
python3 papers/minutes-and-mandates/pilot/scripts/labeling.py export
```

That overwrites `sheet.csv`. Do not run it after you have started labeling unless you intend to discard the passes.
